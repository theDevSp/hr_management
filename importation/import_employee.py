		...
		if self.file_type == 'CSV':
			...
			for row_no in range(sheet.nrows):
				...
				if row_no <= 0:
					...
					employee = self.env['hr.employee'].search([('cin','=',self._correct_cin_format(values.get('cin')).upper())])
					if employee:
						employee[0].contract_id.pp_personnel_id_many2one.write({
							'autoriz_zero_cp':True
						})
