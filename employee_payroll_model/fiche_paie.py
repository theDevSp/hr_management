...
class fiche_paie(models.Model):
    ...
    contract_id = fields.Many2one(
        "hr.contract", string="Contrat", required=True)
    type_emp = fields.Selection(
        related="contract_id.type_emp", string=u"Type d'employé", store=True, readonly=True)
    profile_paie_id = fields.Many2one(
        related="contract_id.profile_paie_id", readonly=True)
    job_id = fields.Many2one(related="contract_id.job_id",
                             string='Poste', store=True, readonly=True)
    type_profile_related = fields.Selection(
        related="contract_id.type_profile_related", string=u"Type du profile", readonly=True)
    salaire_actuel = fields.Float(
        related="contract_id.salaire_actuel", string='Salaire Actuel', store=True, readonly=True)
    date_start = fields.Date(
        related='contract_id.date_start', string="Date d'embauche", readonly=True)
    entreprise_id = fields.Many2one(
        related='contract_id.entreprise_id', readonly=True, store=True)
    ...
    cp_number = fields.Float('Nombre Jours Compensation',
                             compute="_compute_cp_number", store=True)
    ...
    @api.depends('augementation_lines.augmentation_montant_valide')
    ...
    def get_employee_augementations(self):
        augementations = self.env['hr.augmentation'].search(
            [('employee_id', '=', self.employee_id.id), ('state', '=', 'acceptee'), ('type', '!=', 'aug')])
    ...
    def _compute_cp_number(self):
        for rec in self:
            ...
            if not max_worked_days_p:
                ...
                if rec.autoriz_cp:
                    rec.cp_number = min(rec.employee_result()[
                                        'j_comp'], self.employee_id.panier_conge) if rec.employee_result() else 0
                elif rec.autoriz_zero_cp:
                    rec.cp_number = rec.employee_result(
                    )['j_comp'] if rec.employee_result() else 0
                else:
                    rec.cp_number = 0
                ...
                if rec.autoriz_cp:
                    rec.cp_number += min(rec.employee_result()['j_comp']-rec.employee_result()[
                                         'default_day_2_add'], self.employee_id.panier_conge) if rec.employee_result() else 0
                elif rec.autoriz_zero_cp:
                    rec.cp_number += rec.employee_result()['j_comp']-rec.employee_result()[
                        'default_day_2_add'] if rec.employee_result() else 0
    ...
    def compute_total(self):
        for rec in self:
            if rec.contract_id:
                rec.total = rec.nbr_heure_travaille * \
                    rec.salaire_heure if rec.type_profile_related == "h" else rec.nbr_jour_travaille * rec.salaire_jour
    ...
    def employee_result(self):
        ...
        if self.contract_id and self.period_id:
            ...
            joe = self.contract_id.nbre_jour_worked_par_mois_related if self.contract_id.definition_nbre_jour_worked_par_mois_related == 'nbr_saisie' else self.period_id.get_number_of_days_per_month()
            # jour de réguralisation pour les mois exceptionnels (24-25-27)
            default_day_2_add = joe - \
                jom if self.contract_id.definition_nbre_jour_worked_par_mois_related == 'nbr_saisie' else 0
            # heure de travail sur lesquelles le salaire de base de l'employé est définis
            hnt = joe * self.contract_id.nbre_heure_worked_par_jour_related
            # heure de travail sur lesquelles le salaire de base de l'employé est définis
            hntj = self.contract_id.nbre_heure_worked_par_jour_related
            ...
            # jour/heure de compensation de salaire
            j_comp = joe - jot if type_profile == 'j' else ht_equi_days
